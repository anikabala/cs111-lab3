...
struct hash_table_v1 *hash_table_v1_create()
{
	...
	errno = pthread_mutex_init(&hash_table->mutex, NULL);
	if(errno){ exit(errno);}
	return hash_table;
}
...
void hash_table_v1_add_entry(struct hash_table_v1 *hash_table, const char *key, uint32_t value)
{
	...
	errno = pthread_mutex_lock(&hash_table->mutex);
	if(errno){ exit(errno);}
	/* Update the value if it already exists */
	if (list_entry != NULL) {
		list_entry->value = value;
		errno = pthread_mutex_unlock(&hash_table->mutex);
		if(errno){ exit(errno);}
		return;
	}
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table)
{
	...
	errno = pthread_mutex_destroy(&hash_table->mutex);
	if(errno){ exit(errno);}
	free(hash_table);
}
